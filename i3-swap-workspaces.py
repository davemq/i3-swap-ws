...
@click.command()
@click.option('-p', '--prev', 'previous', help='Swap with previous output', is_flag=True, default=False)
def swap_workspaces(previous):
    logging.debug(f'in swap_workspaces(reversed={previous})')
    ...
    # - sort by rect.x
    outputs.sort(key = lambda o: o.rect.x, reverse=previous)
